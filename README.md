...
| 插件版本 | 适配的 DSH |
|---|---|
| 0.1.7 | **DSH 0.1.2-rc.1+**（settings 新 API、原生回合摘要行、中文状态文案） |
| ≤ 0.1.6 | DSH 0.1.1.x |

0.1.7 起不再依赖被移除的 `@deepseek-ai/dsh-settings` 导出（宿主半通过可选消费者接线），但客户端服务注入基于 0.1.2 的 `dsh-client-modules`，未在 0.1.1 上验证。0.1.1 用户请继续使用 0.1.6。
